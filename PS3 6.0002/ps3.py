...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # Creates a dictionary with tuple keys that refer to tiles and values that refer to the amount of dirt on that tile
        self.tiles = {}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #Finds the tile under the position pos
        tile = (int(pos.get_x()), int(pos.get_y()))
        # if the capacity is greater than the amount of dirt on the tile, make the amount of dirt equal to 0.
        # Otherwise, subtract capacity from the amount of dirt on the tile
        if capacity> self.tiles[tile]:
            self.tiles[tile]=0
        else:
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        #if the Tile is clean, return true. Else, return false
        if self.tiles[(m,n)] == 0:
            return True
        else:
            return False
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        cleanTiles = 0
        # cheks every tile. If clean, adds 1 to cleanTiles
    ...
    def is_position_in_room(self, pos):
        ...
        """
        #checks if the x and y values of pos are both greater than 0 and less than the self.height and self.width, respectively
        if pos.get_x() < 0 or pos.get_y() < 0 or pos.get_x() > self.width or pos.get_y()>self.height:
            return False
        else:
            return True
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        # generates a random direction for the robot
        self.direction = random.randint(0,359) + random.random()
        # generates a random position between (0,0) and (room.width, room.height)
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        while True:
            pos = Position(random.randint(0,self.width-1) + random.random(), random.randint(0,self.height-1) + random.random())
            if self.is_position_furnished(pos) == False:
                return pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        newPos = self.position.get_new_position(self.get_robot_direction(), self.speed)
        if self.room.is_position_valid(newPos)== True:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for num in range(num_trials):
        percentClean = 0
        steps = 0
        while min_coverage > percentClean:
            for robot in robots:
                robot.update_position_and_clean()
                percentClean = room.get_num_cleaned_tiles()/room.get_num_tiles()
            steps+=1
        stepsPerTrial.append(steps)
    avg = sum(stepsPerTrial)/len(stepsPerTrial)
    return avg
...
#       of a 20x20 room?
#  With less robots, the difference is fairly stark. The faulty robots take much more time to clean the 20x20 room
#  However, as we add more robots, the difference between the time it takes the standard robots and the faulty robots to clean the room gets smaller.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#  Throughout all the trials, the faulty robots takes significantly longer than the standard robots. However, as the aspect ratio of the room increases, the faulty robot does much worse but the standard robot stays relatively consistent
#
